...
class LocalClassLoaderTest {
    ...
    fun `test loading of local jar test interface`() {
        ...
        val pathToJarDirectory = "src/test/resources"
        // when
        val classLoaders = localClassLoader.createClassLoaderForEachJarInDirectory(pathToJarDirectory)
        // then
        ...
        assert(testInterfaces.first().test() == "Test") { "The test function should return the expected string 'Test'." }
    }
}